...
(function (SnakeCollision) {
    ...
    function hndLoad(_event) {
        ...
        snake = new SnakeCollision.Snake();
        ...
        SnakeCollision.viewport = new f.Viewport();
        SnakeCollision.viewport.initialize("Viewport", gameNode, cmpCamera, canvas);
        f.Debug.log(SnakeCollision.viewport);
        document.addEventListener("keydown", hndKeypress);
        f.Loop.addEventListener("loopFrame" /* LOOP_FRAME */, update);
        f.Loop.start(f.LOOP_MODE.TIME_REAL, 2);
    }
    function update(_event) {
        SnakeCollision.viewport.draw();
